<?php
	...
	pre();
		...
		foreach($query as $result){
			...
			$id = $result['id'];
			$subq = $db->queryRow("SELECT w.datum FROM programma AS w, opstelling_nieuw AS o, spelers AS s WHERE o.wedid = w.id AND o.spelerid = s.id AND o.spelerid = '$id' ORDER BY w.datum ASC LIMIT 1");
			...
			$dpd = strtotime($dpd);
			$d = ($dpd - $gbd) / (24 * 60 * 60);
			//print date("d/m/Y", $gbd);
			//$data = GetDateDifference(date("d/m/Y", $gbd), date("d/m/Y", $dpd));
      $aDays[$id] = $d;
      $aDiff[$id] = age($result['Geboortedatum'], $subq['datum']);
      $aPlayers[$id] = array('naam' => $result['Voornaam']." ".$result['Achternaam']);
			//if($d<7000 and $d > 0){
			//	print $result['Voornaam']." ".$result['Achternaam'];
				//print ": $d dagen<br>";
			//}
		}
