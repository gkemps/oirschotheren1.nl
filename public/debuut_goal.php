<?php
	...
	pre();
		...
		foreach($query as $result){
			...
			$id = $result['id'];
			$subq = $db->queryRow("SELECT w.datum FROM programma AS w, goals AS g, spelers AS s WHERE g.wedid = w.id AND g.maker = s.id AND g.maker = '$id' ORDER BY w.datum ASC LIMIT 1");
